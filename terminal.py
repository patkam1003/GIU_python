...
class Miniterm(object):
    ...
    def __init__(self, serial_instance, echo=False, eol='crlf', filters=()):
        ...
        self.rx_deque = collections.deque(maxlen=1024)
        self.tx_deque = collections.deque(maxlen=1024) 
        self.tx_sem = asyncio.Semaphore(0)
        ...
        try:
            while self.alive:
                ...
                try:
                    ...
                    #async with self.tx_sem:
                    self.tx_sem.acquire()
                    try:
                        if len(self.tx_deque):
                            c = self.tx_deque.popleft()
                        else:
                            c = ''
                    finally:
                        pass
    ...
    def serial_send(self, _data):
        #self.tx_deque.append(_data)
        #self.tx_sem.release()
        self.serial.write(self.tx_encoder.encode(_data))

    def serial_receive(self):
        if len(self.rx_deque):
            return self.rx_deque.popleft()
        else:
            return None
...
def main(default_port=None, default_baudrate=9600, default_rts=None, default_dtr=None, serial_instance=None):
    ...
    miniterm.set_rx_encoding(args.serial_port_encoding) #"UTF-8"
